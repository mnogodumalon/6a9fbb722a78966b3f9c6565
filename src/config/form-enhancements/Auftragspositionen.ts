...
export const formEnhancements: FormEnhancements = {
  ...
  computed: {
    'positionsbetrag_netto': { op: 'mul', left: { kind: 'field', key: 'menge' }, right: { kind: 'field', key: 'einzelpreis_netto' } },
    'positionsbetrag_brutto': (_fields, ctx) => {
      const netto = ctx.num('positionsbetrag_netto');
      const satzKey = ctx.lookupKey('mehrwertsteuersatz');
      const satz = satzKey === 'mwst_7' ? 0.07 : satzKey === 'mwst_19' ? 0.19 : 0;
      return netto * (1 + satz);
    },
  },
};
...
export const computedDeps: Record<string, string[]> = {
  'positionsbetrag_brutto': ['positionsbetrag_netto', 'mehrwertsteuersatz'],
};
