...
import type { Rechnungen, Auftraege, LookupValue } from '@/types/app';
...
export function RechnungenDialog({ open, onClose, onSubmit, defaultValues, recordId, auftraegeList, enablePhotoScan = true, enablePhotoLocation = true }: RechnungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['rechnungsnummer', 'rechnungsdatum', 'faelligkeitsdatum', 'auftrag', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'rechnungsdatum': (
      <div key="rechnungsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="rechnungsdatum"
          placeholder="Wann ist die Rechnung ausgestellt?"
          mode="date"
    ...
    'faelligkeitsdatum': (
      <div key="faelligkeitsdatum" className="space-y-1.5">
        <Label htmlFor="faelligkeitsdatum">{fieldLabel('rechnungen', 'faelligkeitsdatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
        ...
        {showErrors && !fields.faelligkeitsdatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('rechnungen', 'faelligkeitsdatum')}</p>
        )}
      </div>
    ),
    'auftrag': (
    ...
    'mehrwertsteuerbetrag': (
      <div key="mehrwertsteuerbetrag" className="space-y-1.5">
        <Label htmlFor="mehrwertsteuerbetrag">{fieldLabel('rechnungen', 'mehrwertsteuerbetrag')}</Label>
        <Input
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('rechnungen', 'status', 'ueberfaellig') ?? 'Überfällig'}
          </button>
        </div>
    ...
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        <Label htmlFor="bemerkung">{fieldLabel('rechnungen', 'bemerkung')}</Label>
        <Textarea
          id="bemerkung"
          placeholder="Zahlungsbedingungen, Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"rechnungsnummer": "Rechnungsnummer", "rechnungsdatum": "Rechnungsdatum", "faelligkeitsdatum": "Fälligkeitsdatum", "auftrag": "Auftrag", "nettobetrag": "Nettobetrag (€)", "mehrwertsteuerbetrag": "Mehrwertsteuerbetrag (€)", "bruttobetrag": "Bruttobetrag (€)", "status": "Status", "bemerkung": "Bemerkung"};
  ...
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag": {"auftragsnummer": "Auftragsnummer", "auftragsdatum": "Auftragsdatum", "lieferdatum": "Gewünschtes Lieferdatum", "kunde": "Kunde", "status": "Status", "nettobetrag": "Nettobetrag (€)", "mehrwertsteuerbetrag": "Mehrwertsteuerbetrag (€)", "bruttobetrag": "Bruttobetrag (€)", "bemerkung": "Bemerkung"}};
