...
export function KundenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KundenDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'firmenname': (
      <div key="firmenname" className="space-y-1.5">
        <Label htmlFor="firmenname">{fieldLabel('kunden', 'firmenname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="firmenname"
        ...
        {showErrors && !fields.firmenname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kunden', 'firmenname')}</p>
        )}
      </div>
    ),
    ...
    'strasse': (
      <div key="strasse" className="space-y-1.5">
        ...
        <Input
          id="strasse"
          placeholder="z. B. Kurfürstendamm"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        ...
        <Input
          ...
          type="email"
          inputMode="email"
          placeholder="z. B. marco@pizzeria-roma.de"
          value={fields.email ?? ''}
    ...
    'zahlungsziel_tage': (
      <div key="zahlungsziel_tage" className="space-y-1.5">
        <Label htmlFor="zahlungsziel_tage">{fieldLabel('kunden', 'zahlungsziel_tage')}</Label>
        <Input
          id="zahlungsziel_tage"
          type="number"
          inputMode="decimal"
          step="any"
          {...numberInputProps(formEnhancements, 'zahlungsziel_tage')}
          placeholder="z. B. 14"
          value={fields.zahlungsziel_tage !== undefined ? fields.zahlungsziel_tage : (computedValues['zahlungsziel_tage'] ?? '')}
          onChange={e => setFields(f => ({ ...f, zahlungsziel_tage: clampNumberValue(formEnhancements, 'zahlungsziel_tage', e.target.value) }))}
        />
      </div>
    ),
    'kreditlimit': (
      <div key="kreditlimit" className="space-y-1.5">
        <Label htmlFor="kreditlimit">{fieldLabel('kunden', 'kreditlimit')}</Label>
        <Input
          id="kreditlimit"
          type="number"
          inputMode="decimal"
          step="any"
          {...numberInputProps(formEnhancements, 'kreditlimit')}
          placeholder="z. B. 5000"
          value={fields.kreditlimit !== undefined ? fields.kreditlimit : (computedValues['kreditlimit'] ?? '')}
          onChange={e => setFields(f => ({ ...f, kreditlimit: clampNumberValue(formEnhancements, 'kreditlimit', e.target.value) }))}
        />
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Besonderheiten, bevorzugte Kontaktzeiten..."
  ...
  const CURRENCY_KEYS = new Set<string>(["kreditlimit"]);
