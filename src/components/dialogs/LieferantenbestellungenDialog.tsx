/**
 * LieferantenbestellungenDialog — pre-generated create/edit dialog for Lieferantenbestellungen.
 *
 ...
 * LieferantenbestellungenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Lieferantenbestellungen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<LieferantenbestellungenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Lieferantenbestellungen, Lieferanten, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Lieferantenbestellungen';
...
/** Widened prefill type for LieferantenbestellungenDialog.defaultValues — see file header. */
export type LieferantenbestellungenDialogDefaults = Omit<Lieferantenbestellungen['fields'], 'status'> & {
    status?: LookupValue | string;
  };

interface LieferantenbestellungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Lieferantenbestellungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: LieferantenbestellungenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['lieferantenbestellungen']?.['status'] ?? [],
};
...
export function LieferantenbestellungenDialog({ open, onClose, onSubmit, defaultValues, recordId, lieferantenList, enablePhotoScan = true, enablePhotoLocation = true }: LieferantenbestellungenDialogProps) {
  const [fields, setFields] = useState<Partial<Lieferantenbestellungen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bestellnummer', 'bestelldatum', 'lieferant', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Lieferantenbestellungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'lieferantenbestellungen');
      await onSubmit(clean as Lieferantenbestellungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["lieferant"]);
        ...
        return merged as Partial<Lieferantenbestellungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('lieferantenbestellungen') })
    : t('new_entity', { entity: appLabel('lieferantenbestellungen') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'bestellnummer': (
      <div key="bestellnummer" className="space-y-1.5">
        <Label htmlFor="bestellnummer">{fieldLabel('lieferantenbestellungen', 'bestellnummer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="bestellnummer"
          placeholder="z. B. LB-2026-001"
          value={fields.bestellnummer ?? ''}
          onChange={e => setFields(f => ({ ...f, bestellnummer: e.target.value }))}
          required
        />
        {showErrors && !fields.bestellnummer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('lieferantenbestellungen', 'bestellnummer')}</p>
        )}
      </div>
    ),
    'bestelldatum': (
      <div key="bestelldatum" className="space-y-1.5">
        <Label htmlFor="bestelldatum">{fieldLabel('lieferantenbestellungen', 'bestelldatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="bestelldatum"
          placeholder="Wann wurde bestellt?"
          mode="date"
          value={fields.bestelldatum ?? null}
          onChange={v => setFields(f => ({ ...f, bestelldatum: v ?? undefined }))}
          required
        />
        {showErrors && !fields.bestelldatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('lieferantenbestellungen', 'bestelldatum')}</p>
        )}
      </div>
    ),
    'lieferant': (
      <div key="lieferant" className="space-y-1.5">
        <Label htmlFor="lieferant">{fieldLabel('lieferantenbestellungen', 'lieferant')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="lieferant"
          placeholder="Aus der Liste wählen"
        ...
        />
        {showErrors && !fields.lieferant && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('lieferantenbestellungen', 'lieferant')}</p>
        )}
      </div>
    ),
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('lieferantenbestellungen', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('lieferantenbestellungen', 'status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'bestellt'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'bestellt' ? undefined : 'bestellt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'bestellt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('lieferantenbestellungen', 'status', 'bestellt') ?? 'Bestellt'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('lieferantenbestellungen', 'status', 'eingegangen') ?? 'Eingegangen'}
          </button>
        </div>
        {showErrors && !fields.status && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('lieferantenbestellungen', 'status')}</p>
        )}
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        <Label htmlFor="bemerkung">{fieldLabel('lieferantenbestellungen', 'bemerkung')}</Label>
        <Textarea
          id="bemerkung"
          placeholder="Besondeheiten dieser Bestellung..."
  ...
  const FIELD_LABELS: Record<string, string> = {"bestellnummer": "Bestellnummer", "bestelldatum": "Bestelldatum", "lieferant": "Lieferant", "status": "Status", "bemerkung": "Bemerkung"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"lieferant": {"firmenname": "Firmenname", "ansprechpartner_vorname": "Ansprechpartner Vorname", "ansprechpartner_nachname": "Ansprechpartner Nachname", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "PLZ", "ort": "Ort", "email": "E-Mail", "telefon": "Telefon", "website": "Website", "bemerkung": "Bemerkung"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.LIEFERANTENBESTELLUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
