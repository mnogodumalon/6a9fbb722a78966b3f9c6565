/**
 * LieferantenDialog — pre-generated create/edit dialog for Lieferanten.
 *
 ...
 * LieferantenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Lieferanten['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<LieferantenDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Lieferanten';
...
/** Widened prefill type for LieferantenDialog.defaultValues — see file header. */
export type LieferantenDialogDefaults = Lieferanten['fields'];
...
interface LieferantenDialogProps {
  ...
  defaultValues?: LieferantenDialogDefaults;
...
export function LieferantenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: LieferantenDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Lieferanten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'lieferanten');
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('lieferanten') })
    : t('new_entity', { entity: appLabel('lieferanten') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'firmenname': (
      <div key="firmenname" className="space-y-1.5">
        <Label htmlFor="firmenname">{fieldLabel('lieferanten', 'firmenname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="firmenname"
          placeholder="z. B. Schmidt Großhandel"
          value={fields.firmenname ?? ''}
          onChange={e => setFields(f => ({ ...f, firmenname: e.target.value }))}
          required
        />
        {showErrors && !fields.firmenname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('lieferanten', 'firmenname')}</p>
        )}
      </div>
    ),
    'ansprechpartner_vorname': (
      <div key="ansprechpartner_vorname" className="space-y-1.5">
        <Label htmlFor="ansprechpartner_vorname">{fieldLabel('lieferanten', 'ansprechpartner_vorname')}</Label>
        <Input
    ...
    'ansprechpartner_nachname': (
      <div key="ansprechpartner_nachname" className="space-y-1.5">
        <Label htmlFor="ansprechpartner_nachname">{fieldLabel('lieferanten', 'ansprechpartner_nachname')}</Label>
        <Input
    ...
    'strasse': (
      <div key="strasse" className="space-y-1.5">
        <Label htmlFor="strasse">{fieldLabel('lieferanten', 'strasse')}</Label>
        <Input
          id="strasse"
    ...
    'hausnummer': (
      <div key="hausnummer" className="space-y-1.5">
        <Label htmlFor="hausnummer">{fieldLabel('lieferanten', 'hausnummer')}</Label>
        <Input
          id="hausnummer"
          placeholder="z. B. 42"
    ...
    'plz': (
      <div key="plz" className="space-y-1.5">
        <Label htmlFor="plz">{fieldLabel('lieferanten', 'plz')}</Label>
        <Input
          id="plz"
    ...
    'ort': (
      <div key="ort" className="space-y-1.5">
        <Label htmlFor="ort">{fieldLabel('lieferanten', 'ort')}</Label>
        <Input
          id="ort"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('lieferanten', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
          placeholder="z. B. info@schmidt-gros.de"
          value={fields.email ?? ''}
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('lieferanten', 'telefon')}</Label>
        <Input
          id="telefon"
          type="tel"
          inputMode="tel"
    ...
    'website': (
      <div key="website" className="space-y-1.5">
        <Label htmlFor="website">{fieldLabel('lieferanten', 'website')}</Label>
        <Input
          id="website"
          type="url"
          inputMode="url"
          placeholder="z. B. www.schmidt-gros.de"
    ...
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        <Label htmlFor="bemerkung">{fieldLabel('lieferanten', 'bemerkung')}</Label>
        <Textarea
          id="bemerkung"
          placeholder="Besonderheiten, Lieferbedingungen, Öffnungszeiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"firmenname": "Firmenname", "ansprechpartner_vorname": "Ansprechpartner Vorname", "ansprechpartner_nachname": "Ansprechpartner Nachname", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "PLZ", "ort": "Ort", "email": "E-Mail", "telefon": "Telefon", "website": "Website", "bemerkung": "Bemerkung"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.LIEFERANTEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
