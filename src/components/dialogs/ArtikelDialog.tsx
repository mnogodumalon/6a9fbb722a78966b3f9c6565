/**
 * ArtikelDialog — pre-generated create/edit dialog for Artikel.
 *
 ...
 * ArtikelDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Artikel['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<ArtikelDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Artikel, Lieferanten, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Artikel';
...
/** Widened prefill type for ArtikelDialog.defaultValues — see file header. */
export type ArtikelDialogDefaults = Omit<Artikel['fields'], 'mehrwertsteuersatz'> & {
...
interface ArtikelDialogProps {
  ...
  defaultValues?: ArtikelDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  mehrwertsteuersatz: LOOKUP_OPTIONS['artikel']?.['mehrwertsteuersatz'] ?? [],
};
...
export function ArtikelDialog({ open, onClose, onSubmit, defaultValues, recordId, lieferantenList, enablePhotoScan = true, enablePhotoLocation = true }: ArtikelDialogProps) {
  ...
  const REQUIRED_FIELDS = ['artikelnummer', 'bezeichnung', 'mehrwertsteuersatz'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Artikel['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'artikel');
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('artikel') })
    : t('new_entity', { entity: appLabel('artikel') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'artikelnummer': (
      <div key="artikelnummer" className="space-y-1.5">
        <Label htmlFor="artikelnummer">{fieldLabel('artikel', 'artikelnummer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="artikelnummer"
          placeholder="z. B. ART-2026-001"
          value={fields.artikelnummer ?? ''}
          onChange={e => setFields(f => ({ ...f, artikelnummer: e.target.value }))}
          required
        />
        {showErrors && !fields.artikelnummer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('artikel', 'artikelnummer')}</p>
        )}
      </div>
    ),
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        <Label htmlFor="bezeichnung">{fieldLabel('artikel', 'bezeichnung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="bezeichnung"
          placeholder="z. B. Mozarella 500g"
          value={fields.bezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, bezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.bezeichnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('artikel', 'bezeichnung')}</p>
        )}
      </div>
    ),
    'einkaufspreis': (
      <div key="einkaufspreis" className="space-y-1.5">
        <Label htmlFor="einkaufspreis">{fieldLabel('artikel', 'einkaufspreis')}</Label>
        <Input
          id="einkaufspreis"
          type="number"
          inputMode="decimal"
    ...
    'verkaufspreis': (
      <div key="verkaufspreis" className="space-y-1.5">
        <Label htmlFor="verkaufspreis">{fieldLabel('artikel', 'verkaufspreis')}</Label>
        <Input
          id="verkaufspreis"
          type="number"
          inputMode="decimal"
    ...
    'mehrwertsteuersatz': (
      <div key="mehrwertsteuersatz" className="space-y-1.5">
        <Label htmlFor="mehrwertsteuersatz">{fieldLabel('artikel', 'mehrwertsteuersatz')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('artikel', 'mehrwertsteuersatz', 'mwst_7') ?? '7 %'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('artikel', 'mehrwertsteuersatz', 'mwst_19') ?? '19 %'}
          </button>
        </div>
        {showErrors && !fields.mehrwertsteuersatz && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('artikel', 'mehrwertsteuersatz')}</p>
        )}
      </div>
    ),
    'lagerbestand': (
      <div key="lagerbestand" className="space-y-1.5">
        <Label htmlFor="lagerbestand">{fieldLabel('artikel', 'lagerbestand')}</Label>
        <Input
          id="lagerbestand"
          type="number"
          inputMode="decimal"
    ...
    'mindestbestand': (
      <div key="mindestbestand" className="space-y-1.5">
        <Label htmlFor="mindestbestand">{fieldLabel('artikel', 'mindestbestand')}</Label>
        <Input
          id="mindestbestand"
          type="number"
          inputMode="decimal"
    ...
    'lieferant': (
      <div key="lieferant" className="space-y-1.5">
        <Label htmlFor="lieferant">{fieldLabel('artikel', 'lieferant')}</Label>
        <Combobox
          id="lieferant"
    ...
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        <Label htmlFor="bemerkung">{fieldLabel('artikel', 'bemerkung')}</Label>
        <Textarea
          id="bemerkung"
          placeholder="Lagerkennzeichen, Lagerort, besondere Hinweise..."
  ...
  const FIELD_LABELS: Record<string, string> = {"artikelnummer": "Artikelnummer", "bezeichnung": "Bezeichnung", "einkaufspreis": "Einkaufspreis (€)", "verkaufspreis": "Verkaufspreis (€)", "mehrwertsteuersatz": "Mehrwertsteuersatz", "lagerbestand": "Aktueller Lagerbestand", "mindestbestand": "Mindestbestand", "lieferant": "Lieferant", "bemerkung": "Bemerkung"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.ARTIKEL} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
