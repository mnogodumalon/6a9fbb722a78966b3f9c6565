...
export function AuftraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: AuftraegeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['auftragsnummer', 'auftragsdatum', 'kunde', 'status'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        if (kundeName) {
          const kundeMatch = kundenList.find(r => matchName(kundeName!, [String(r.fields.firmenname ?? '')]));
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'auftragsdatum': (
      <div key="auftragsdatum" className="space-y-1.5">
        <Label htmlFor="auftragsdatum">{fieldLabel('auftraege', 'auftragsdatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="auftragsdatum"
        ...
        {showErrors && !fields.auftragsdatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('auftraege', 'auftragsdatum')}</p>
        )}
      </div>
    ),
    'lieferdatum': (
      <div key="lieferdatum" className="space-y-1.5">
        <Label htmlFor="lieferdatum">{fieldLabel('auftraege', 'lieferdatum')}</Label>
        <DatePicker
          id="lieferdatum"
    ...
    'kunde': (
      <div key="kunde" className="space-y-1.5">
        ...
        {showErrors && !fields.kunde && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('auftraege', 'kunde')}</p>
        )}
      </div>
    ),
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('auftraege', 'status', 'entwurf') ?? 'Entwurf'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('auftraege', 'status', 'freigegeben') ?? 'Freigegeben'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'geliefert'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'geliefert' ? undefined : 'geliefert') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'geliefert'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('auftraege', 'status', 'geliefert') ?? 'Geliefert'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('auftraege', 'status', 'storniert') ?? 'Storniert'}
          </button>
        </div>
    ...
    'nettobetrag': (
      <div key="nettobetrag" className="space-y-1.5">
        <Label htmlFor="nettobetrag">{fieldLabel('auftraege', 'nettobetrag')}</Label>
        <Input
          id="nettobetrag"
          type="number"
          inputMode="decimal"
    ...
    'mehrwertsteuerbetrag': (
      <div key="mehrwertsteuerbetrag" className="space-y-1.5">
        <Label htmlFor="mehrwertsteuerbetrag">{fieldLabel('auftraege', 'mehrwertsteuerbetrag')}</Label>
        <Input
          id="mehrwertsteuerbetrag"
          type="number"
          inputMode="decimal"
          step="any"
          {...numberInputProps(formEnhancements, 'mehrwertsteuerbetrag')}
          placeholder="z. B. 47,50"
          value={fields.mehrwertsteuerbetrag !== undefined ? fields.mehrwertsteuerbetrag : (computedValues['mehrwertsteuerbetrag'] ?? '')}
          onChange={e => setFields(f => ({ ...f, mehrwertsteuerbetrag: clampNumberValue(formEnhancements, 'mehrwertsteuerbetrag', e.target.value) }))}
        />
      </div>
    ),
    'bruttobetrag': (
      <div key="bruttobetrag" className="space-y-1.5">
        <Label htmlFor="bruttobetrag">{fieldLabel('auftraege', 'bruttobetrag')}</Label>
        <Input
          id="bruttobetrag"
          type="number"
          inputMode="decimal"
    ...
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        <Label htmlFor="bemerkung">{fieldLabel('auftraege', 'bemerkung')}</Label>
        <Textarea
          id="bemerkung"
          placeholder="Bearbeitungsnotizen, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"auftragsnummer": "Auftragsnummer", "auftragsdatum": "Auftragsdatum", "lieferdatum": "Gewünschtes Lieferdatum", "kunde": "Kunde", "status": "Status", "nettobetrag": "Nettobetrag (€)", "mehrwertsteuerbetrag": "Mehrwertsteuerbetrag (€)", "bruttobetrag": "Bruttobetrag (€)", "bemerkung": "Bemerkung"};
  const CURRENCY_KEYS = new Set<string>(["nettobetrag", "mehrwertsteuerbetrag", "bruttobetrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"firmenname": "Firmenname", "ansprechpartner_vorname": "Ansprechpartner Vorname", "ansprechpartner_nachname": "Ansprechpartner Nachname", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "PLZ", "ort": "Ort", "email": "E-Mail", "telefon": "Telefon", "zahlungsziel_tage": "Zahlungsziel (Tage)", "kreditlimit": "Kreditlimit (€)", "bemerkung": "Bemerkung"}};
    ...
    {createKundenOpen && (
      <KundenDialog
        ...
        defaultValues={createKundenInitial
          ? ({ firmenname: createKundenInitial } as any)
          : undefined}
      />
    )}
    </>
